...
> Источник: [TheSmokeDev/geo-skills](https://github.com/TheSmokeDev/geo-skills) (MIT). Скопированы только 6 скилов из полного репозитория (без prospect/CRM/фабрики страниц).
>
> **Неполный набор субагентов:** из 5 субагентов ниже установлены только 3 (`geo-citability`, `geo-crawlers`+`geo-llmstxt`, `geo-schema`) — `geo-brand-mentions` и `geo-content` не копировались. Composite GEO Score считать по 4 из 6 категорий (Citability 25% + Technical 15% + Schema 10% + доступные части Platform Optimization), явно указывая, что Brand Authority (20%) и Content E-E-A-T (20%) не пройдены аудитом.
>
> **JSON-LD:** генерация новой разметки для goodmi.ru — см. предупреждение в `geo-schema/SKILL.md`, не через этот скилл.
