...
> Источник: [TheSmokeDev/geo-skills](https://github.com/TheSmokeDev/geo-skills) (MIT). Скопировано вручную, без остального репозитория.
>
> **GOODMi-специфичное предупреждение — читать перед применением раздела "JSON-LD Generation":**
> Корневой `AGENTS.md` проекта прямо запрещает `@graph` в JSON-LD (несовместимо с CS-Cart) и `FAQPage` (Google убрал rich results для коммерческих сайтов, вызывает ошибки GSC), а `WebPage` CS-Cart генерирует сам — добавлять вручную нельзя. Раздел "Step 6: JSON-LD Generation" ниже рекомендует именно `@graph`-паттерн и `FAQPage` — **эти рекомендации для goodmi.ru не применять**. Использовать этот скилл только для **аудита/скоринга** существующей разметки (Step 1-5) и справочника (`sameAs`-стратегия, устаревшие схемы), не для генерации новых блоков — генерация JSON-LD для goodmi.ru всегда через `seo-shop-page-builder-v2` / `seo-meta-builder-v2`, которые уже соблюдают эти запреты.
